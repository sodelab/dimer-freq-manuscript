...
#1MR dimer Gradient
D1G=( `grep -A 50 "# 1MR" mavi/dimer-intra/3MR/001.hs | grep -A 8 "# Gradient / hartree" | tail -8 | awk '{print $2}'` )
#1MR dimer Hessian
D1H=( `grep -A 50 "# 1MR" mavi/dimer-intra/3MR/001.hs | grep -A 8 "Hessian(i,i)" | tail -8 | awk '{print $2}'` )
#1MR dimer Cubic
D1C=( `grep -A 50 "# 1MR" mavi/dimer-intra/3MR/001.hs | grep -A 8 "Cubic(i,i,i)" | tail -8 | awk '{print $2}'` )
#1MR dimer Quadratic
D1Q=( `grep -A 50 "# 1MR" mavi/dimer-intra/3MR/001.hs | grep -A 8 "Quartic(i,i,i,i)" | tail -8 | awk '{print $2}'` )


#2MR dimer Hessian
D2H=( `grep -A 180 "# 2MR" mavi/dimer-intra/3MR/001.hs | grep -A 28 "Hessian(i,j)" | tail -28 | awk '{print $3}'` )
#2MR dimer Quartic
D2Q1=( `grep -A 180 "# 2MR" mavi/dimer-intra/3MR/001.hs | grep -A 28 "Quartic(i,i,j,j)" | tail -28 | awk '{print $3}'` )
#2MR dimer Cubic
D2C=( `grep -A 180 "# 2MR" mavi/dimer-intra/3MR/001.hs | grep -A 56 "Cubic(i,i,j)" | tail -56 | awk '{print $3}'` )
#2MR dimer Quartic
D2Q2=( `grep -A 180 "# 2MR" mavi/dimer-intra/3MR/001.hs | grep -A 56 "Quartic(i,i,i,j)" | tail -56 | awk '{print $3}'` )


#3MR dimer Cubic
D3C=( `grep -A 250 "# 3MR" mavi/dimer-intra/3MR/001.hs | grep -A 56 "Cubic(i,j,k)" | tail -56 | awk '{print $4}'` )
#3MR dimer Quartic
D3Q=( `grep -A 250 "# 3MR" mavi/dimer-intra/3MR/001.hs | grep -A 168 "Quartic(i,i,j,k)" | tail -168 | awk '{print $4}'` )
...
#sed -i '' "s,\?dimer-QFF-2MR-H\?,${d2mrH}," supp/info.tex
